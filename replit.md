...
- **Activation-First Onboarding**: New users are redirected to an 8-step `/onboarding` wizard before accessing the dashboard. Steps: (1) Product mode selection (cakes/treats/both), (2) Portfolio upload (up to 6 images), (3) Pricing tier selection with auto-seeded calculator config via `POST /api/baker/seed-pricing` and `server/pricing-seed.ts`, (4) Quote simulation preview, (5) DM activation with copyable reply template, (6) Branding (business name, slug, photos), (7) Share order page link (marks `onboarding_completed=true`), (8) Optional Stripe Connect. Each step fits one viewport height. `ProtectedRoute` has `allowDuringOnboarding` flag. Post-onboarding: "Launch Your Bakery" activation checklist on dashboard.
